...
注意`net.minecraft.server.level.DistanceManager.addRegionTicket`是出现在引号里面的，说明这不是某些Mod反混淆日志输出的结果。
