...
这张图是高度简化的，而且不同版本间图中步骤的顺序可能会有细微的的变更，比如在1.17.1中，计划刻步骤被向前移动到了相应维度的刷怪、随机刻与卸载这些步骤的整体之前。尽管近几年的改动很少会对玩家以外的实体运动行为造成显著的影响，但对其有一个清晰的把握在更广泛的领域中的重要性也不言而喻，如果有兴趣的话，读者可以翻一下Fallen_Breath的B站主页看一下相对完整的版本**\[2\]**（需注意适用版本）或直接翻阅源代码。

![](media/image4.png)

> 图2.2.11.16.x服务端每tick运算流程图
...
可以看出，实体由骑乘关系组成了一个个树形结构，每个树形结构中一个分支运算完成后下一个同级分支就会开始运算，且分支内的运算顺序是由"根"到"叶"的。若有相关知识基础，我们也可以用森林的先序遍历或DFS遍历算法来类比这一过程。
