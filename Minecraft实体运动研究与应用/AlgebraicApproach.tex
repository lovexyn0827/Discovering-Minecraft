...
\begin{enumerate}
	...
	\end{enumerate}
	\item Combine the special cases to establish a approximation along with a (hard or probabilistic) bound, for "physically" rigorous analysis.
\end{enumerate}
...
\begin{definition}
	An \index{united algebra}{\bf{united algebra on $A$ with unit $u$}}, notably $A_u$, is an algebraic structure of ordered pairs $<m, u>$, where $m \in A$ , $u\in U$ for some unit group $U$ and $A$ is an algebraic structure. An element in an united algebra is a \index{united value}{\bf{united value}}. Its left and right component are called its \index{magnitude}{\bf{magnitude}} and \index{unit}{\bf{unit}} respectively.
...
\begin{definition}
	...
	$\left(\mathcal t_{f, I_1}(w) \circ \cdots \circ \mathcal t_{f, I_n}\right)(w)$ is the \index{world state $n$ tick(s) after}{\bf{world state $n$ tick(s) after $w$ under $f$ and $I_1, \dots, I_k$}}.
\end{definition}
...
\begin{definition}
	Given a non-definite ticker $\mathcal t_{f, I}(w)$ on $\mathcal W$, a kinematically free entity is a named kinematic entity $<S_0, L_0>$ with name $K\in L$ such that
	
	\begin{equation*}
		\forall n (n\in\mathbb N\implies(\exists S_n\exists L_n\forall w \forall I(w\in \mathcal W \implies <S_n, L_0>\in \mathcal E(\mathcal t^n(w)) \land  S_n\in \mathcal S\land K\in L_n))
	\end{equation*}
\end{definition}

\section{Kinematic Formulas}
...
\section{Prefect Pearl Aligners}
