...
\begin{tikzpicture}[remember picture, overlay]
	...
	\node[scale=2] at ([yshift=2cm, xshift=-4.0cm]current page.south){{\includegraphics[height=1cm]{autumner.png}}};
...
\begin{remark}
	If the right component of an united value is the identity of underlying unit group, it is regarded to be equal to its left component for convenience.
\end{remark}
...
\begin{definition}
	...
	\begin{equation*}
		\mathcal{F}(e, s) = \{m \times 2^k: m, k \in \mathbb{Z} \land |m| < 2^s \land  2^{-2^{e - 1} + 2 - s} \le e \le 2^{2^{e - 1} - s}  \}
	\end{equation*}
	...
	\begin{equation*}
		\hat{\mathcal{F}}(e, s) = \{m \times 2^k: m, k \in \mathbb{Z} \land |m| < 2^ s \land e \ge 2^{-2^{e - 1} + 2 - s}\}
	\end{equation*}
...
\begin{definition}
	...
	\begin{equation*}\mathcal{R}_{\mathcal{F}}(x) = 
		\begin{cases}
			...
			\argmin\limits_{t\in\mathcal{M}(\mathcal{F}, x)} \left(|t| \cdot 2^{s - \left\lfloor \log_2 |t| \right\rfloor + 1} \mod 2\right) & |\mathcal{M}(\mathcal{F}, x)| = 2
		\end{cases}
	\end{equation*}
...
\begin{definition}
	Given a non-definite ticker $\mathcal t_{f, I}(w)$ on $\mathcal W$ and $\mathcal W'\subset W$ then for $N\in \mathcal N$, a \index{kinematically free entity}{\bf{kinematically $N$-free entity in $\mathcal W'$}}, or \index{free entity}{\bf{$N$-free entity in $\mathcal W'$}}, is a kinematic entity $<S_0, L_0>$ with name $K\in L$ such that
	...
	\begin{align*}
		p &= w\in \mathcal W' \land <S_0, L_0>\in E(w)\\
...
\begin{definition}
	Given a non-definite ticker $\mathcal t_{t, I}$ on $\mathcal W$, $w\in \mathcal W'\subset \mathcal W$, and a $N$-free entity $<S, L>\in E(w)$ in $\mathcal W'$, if for all $n\in \mathbb N \land n \le N$, there is and motion operator $f$ such that for the entity $<S_n, L_n>$ sharing the same name as $<S, L>$ in $E(\mathcal t_{f, I_1, \dots, I_n}(w))$, $S_n = f^n(S)$, the free entity is \index{cyclic free entity}{\bf{cyclic}}, and $f$ is the \index{intrinsic operator}{\bf{intrinsic operator} of $<S, L>$ under $\mathcal t_{t, I}$ in $\mathcal W'$}.
...
\begin{definition}
	Given a non-definite ticker $\mathcal t_{t, I}(w)$ on $\mathcal W$, $w\in \mathcal W'\subset \mathcal W$, and a $N$-free entity $<S, L>\in E(w)$ in $\mathcal W'$, for any $n\in \mathbb N$ and $0\le n\le N$, the entity $<S_n, L_n>$ is supposed to share the same name as $<S_0, L_0>$ in $E(\mathcal t_{f, I_1, \dots, I_n}(w))$, $S_n = f^n(S)$, and thus notations $v_n = V(S_n)$ and $x_n = P(S_n)$ are adopted in this section.
	
	For conciseness, unless otherwise stated, intrinsic operators in this section are supposed to be under $\mathcal t_{t, I}$ and in $\mathcal W'$, and the world state(s) where entity states resides in is supposed to be defined upon $S_{\mathbb R, \mathbb R, m, m\cdot gt^{-1}, d}$ where $m$ and $gt$ are meter and game tick respectively and $d\in \mathbb N$.
\end{definition}

\begin{theorem}\label{thm:vel-formula}
	If $<S_0, L_0>$ has an intrinsic operator that is velocitically tail equivalent to $D_k \circ A_I$ for some $k\in R_1$ with $k \ne 1$ and $I \in \mathbb R_{m\cdot gt^{-1}}^d$, then
	
	\begin{equation}
		v_n = k^{n} v_0 + \frac{I(1 - k^{n})}{1 - k}
	\end{equation}
	
	If the intrinsic operator is velocitically tail equivalent to $A_I \circ D_k$ for some $k\in \mathbb R$ with $k \ne 1$ and $I \in \mathbb R_{m\cdot gt^{-1}}^d$, then
...
\begin{theorem}\label{thm:disp-formula}
	If $<S_0, L_0>$ has an intrinsic operator that is positionally tail equivalent to $M_{t_0} \circ D_k \circ A_I$ for some $k\in \mathbb R_1$, $t_0 \in \mathbb R_{gt}$ and $I\in R_{m\cdot gt^{-1}}^d$, then
...
\begin{theorem}\label{thm:vel-convergence}
	If $<S_0, L_0>$ has an intrinsic operator $f$ that is velocitically tail equivalent to $D_k \circ A_I$ or $A_I \circ D_k$ for some $t_0 \in \mathbb R_{gt}$, $k\in \mathbb R_1$ with $|k| \le 1$ and $I\in R_{m\cdot gt^{-1}}^d$, then
...
\begin{theorem}\label{thm:max-disp}
	If $<S_0, L_0>$ has an intrinsic operator $f$ that is positionally tail equivalent to $M_{t_0} \circ D_k \circ A_I$ for some $k, t_0\in \mathbb R_1$ with $|k| \le 1$, $I = (0, at_0, 0)\in R_{m\cdot gt^{-1}}^3$, and $v_0 = (v_{x0}, v_{y0}, v_{z0}) \in \mathbb R_{m\cdot gt^{-1}}^3$, then the second component $x_n$ attains its maximal or minimal value $M$ at
...
\begin{theorem}\label{thm:inverted-kinematic}
	If all entity states in a subset $S$ of the angular entity state space defining $\mathcal W$ have a shared intrinsic operator $f$ that is positionally tail equivalent to $M_{t_0} \circ D_k \circ A_I$ for some $k, t_0\in \mathbb R_1$ with $|k| \le 1$, $I\in R_{m\cdot gt^{-1}}^d$, then given $x\in \mathbb R_{m}$ and $n\in \mathbb N$, there is at most 1 $<S_0, L_0>\in S$ such that $x_n = x$. Further more, 
...
\begin{theorem}\label{thm:trace-theorem}
	If $<S_0, L_0>$ has an intrinsic operator that is positionally tail equivalent to $M_{t_0} \circ D_k \circ A_I$ for some $k\in \mathbb R_1$, $t_0 \in \mathbb R_{gt}$ and $I\in R_{m\cdot gt^{-1}}^d$, then magnitude of all $x_n$'s lays in the same hyperplane in $\mathbb R^d$.
	
	Furthermore, there is a unitary operator $T$ on $\mathbb R^3$ such that $T(\mu(I)) = \left(\left\Vert \mu(I)\right\Vert, 0, \dots, 0\right)$, then all $x_n$'s lays in the same hypersurface
...
\begin{enumerate}
	...
	\item Show that $\lim_{n\to \infty} x_n$ exists if $<S_0, L_0>$ has an intrinsic operator that is positionally tail equivalent to $M_{t_0} \circ D_k$ or $D_k \circ M_{t_0}$ for some $k, t_0\in \mathbb R_1$ but $k\ne 1$. And evaluate the limit.
\end{enumerate}
